...
Content-based recommendation systems suggest items by comparing the attributes of the items themselves. For text-heavy items, this often means representing item descriptions, titles, genres, tags, or categories as numerical vectors and then recommending items with similar vectors. This approach is useful when user behavior data is unavailable, because it can work from item metadata alone.

The main strength of content-based recommendation is interpretability. If two courses share words such as "python", "finance", or "web development", the reason for their similarity is easy to explain. The limitation is that the system may recommend items that are too similar to the selected item and may miss useful alternatives that use different wording.

## Collaborative Filtering

Collaborative filtering recommends items by learning from user behavior. A system can compare users with similar histories or compare items that tend to be chosen by similar users. This approach is powerful when there are enough ratings, clicks, purchases, enrollments, or watch histories because it can discover patterns that are not obvious from item descriptions alone.

Collaborative filtering could not be applied directly in this project because the dataset does not contain user IDs, ratings, enrollments, clicks, or any other interaction records. Without those relationships between users and courses, the system cannot learn user-based or item-based preference patterns.

## Hybrid Recommendation Systems

Hybrid recommendation systems combine more than one recommendation strategy, often using both content features and collaborative filtering. This can reduce the weaknesses of each individual method. For example, content features can help recommend new items with little interaction history, while collaborative signals can capture user preferences that metadata alone may not explain.

In this project, a full hybrid recommender was not possible because user interaction data was missing. However, comparing TF-IDF with KNN still allowed us to study two content-based approaches and observe how recommendations change when numeric metadata is added to text features.
...
|---|---|---|
| Experiment A | `course_title only` | Already produced relevant recommendations because the title contains strong keywords: web, programming, and python. |
| Experiment B | `course_title + subject` | Kept the same top-five recommendation set while improving several similarity scores; subject reinforced Web Development relevance. |
| Experiment C | `course_title + subject + level` | Kept recommendations focused on Python and Web Development while adding useful difficulty context; selected as the best final text representation. |
...
The app uses real project files where available:

| File or Directory | Use in Dashboard |
|---|---|
| `data/processed/processed_udemy_courses.csv` | Course catalog and recommendation input |
| `data/synthetic_users.csv` | Synthetic persona demonstration |
| `results/recommendation_examples/tfidf_knn_evaluation_examples.csv` | Algorithm comparison examples |
| `results/evaluation/` | Overlap, runtime, ablation, and summary results |
| `results/graphs/` | Presentation graphs |
